...
<div id="sod_frm" class="sod_frm_pc">
    <!-- 주문상품 확인 시작 { -->
    <div class="tbl_head03 tbl_wrap od_prd_list" style="display: none">
        <table id="sod_list">
        <thead>
        <tr>
            <th scope="col">상품명</th>
        ...
        // gogosing ------test start
        echo 'gogosing payment test';
        echo '<br><br> $order_action_url : ';
        echo $order_action_url;

        echo '<br><br> 글번호 wr_id :';
        echo $wr_id;

        echo '<br><br> 유료결제 여부 wr_1 :';
        echo $write['wr_1'];

        echo '<br><br> 유료결제 여부 wr_2 :';
        echo $write['wr_2'];

        $s_cart_id = 2022110415031050;
//        $s_cart_id = $wr_id; // 유니크 id 를 생성해서 wr_3 에 저장할것

        echo '<br><br> 쇼핑 카드 번호 $s_cart_id : ';
        echo $s_cart_id;
        // gogosing ------test end
...
//        echo '<br><br> $result : '; echo $result;
//        var_dump($result);
//        echo (empty($result) ? 'empty': $result);
//        print_r($result,true);
        // gogosing ------test end
...
//            $sell_price = $sum['price'];
            $sell_price = $write['wr_2'];;

            // gogosing 가격이 없는경우 1000 원
            if($sell_price<1) {
                $sell_price = 1000;
            }
...
//            alert('장바구니가 비어 있습니다.', G5_SHOP_URL.'/cart.php');
        ...
        // gogosing 유료결제인 경우 금액 예외처리
        if('1' == $write['wr_1']) {
            // Undefined variable 에러 발생하여 임시 define
            $sell_price =0;
            $send_cost = 0;
            $total_send_cost = 0;
            ...
            if( ! $tot_sell_price || 0 == $tot_sell_price) {
                $tot_sell_price = $sell_price;
            }
            if( ! $send_cost || 0 == $send_cost) {
                $send_cost = 0;
            }
    ...
    <!-- gogosing 금액처리 -->
    <?php
        if (!$tot_sell_price || $tot_sell_price < 1000) $tot_sell_price = 1000;
        if (!$send_cost) $send_cost = 0;
    ?>
    ...
    <div class="sod_left">
        ...
        <section id="sod_frm_orderer">
            ...
            <div class="tbl_frm01 tbl_wrap">
                ...
                <tr>
                    <th scope="row"><label for="od_tel">연락처1<strong class="sound_only"> 필수</strong></label></th>
                ...
                <tr>
                    <th scope="row"><label for="od_hp">연락처2</label></th>
...
$(function() {
    ...
    $("#od_settle_bank").on("click", function() {
        ...
        $("#settle_bank").css("border:1px solid #e3e5e8;border-top:0 none;background:#fff;padding:10px;clear:both;line-height:32px;display:block")
        $("#settle_bank").show();
    });
...
function forderform_check(f)
{
    ...
    if( jQuery(f).triggerHandler("form_sumbit_order_"+form_order_method) !== false ) {
        ...
        // gogosing
        f.price.value       = <?php echo $write['wr_1'] ?>
        <?php if($default['de_tax_flag_use']) { ?>
        ...
        f.buyertel.value    = f.od_tel.value ? f.od_tel.value : f.od_hp.value;
        f.recvname.value    = f.od_name.value;
        f.recvtel.value     = f.od_tel.value ? f.od_tel.value : f.od_hp.value;
        f.recvpostnum.value = '';
        f.recvaddr.value    = '';
